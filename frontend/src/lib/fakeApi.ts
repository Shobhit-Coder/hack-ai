...
import { APPS_KEY, JOBS_KEY, CANDIDATES_KEY, SETTINGS_KEY } from "../constants";
...
let jobs = [
  {
    ...
    createdAt: now(),
    descriptionText: "Python/FastAPI",
  },
  {
    id: "j2",
    title: "DevOps Engineer",
    createdAt: now(),
    descriptionText: "Kubernetes/Terraform",
  },
];
let candidates = [
  { id: "c1", fullName: "Rakesh J.", phoneE164: "+9199xxxxxxxx" },
  { id: "c2", fullName: "Priya S.", phoneE164: "+9198xxxxxxxx" },
];
let interviews = [
  {
    id: "iv1",
...
export const publicApi = {
  ...
  async applyForJob(jobId: string, candidate: { fullName: string; phoneE164: string }, file: File){
    await wait(120);
    const apps = lsGet(APPS_KEY, [] as any[]);
    // (demo) read the file as base64 to “store”
    ...
    // (optional) also add candidate to admin list so you can see them
    const created = await adminApi.createCandidate(candidate);
    return created;
  },
};
